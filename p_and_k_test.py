...
# === CONFIG ===
las_file_1 = "240829_ALS_Matrice300_Svb_Classified.las"
...
def idw_interpolate_points_test(x, y, z, power=1, max_neighbors=12):
	max_neighbors += 1
	known_points = np.column_stack((x, y))
    ...
    # Query nearest neighbors
	distances, idxs = tree.query(known_points, k=max_neighbors, workers = -1)
	...
	with np.errstate(divide='ignore'):
		weights = 1.0 / np.power(distances, power)
		weights[distances == 0] = 0  # Assign high weight to exact matches
	...
	return interpolated_vals.reshape(x.shape)
...
best_k = 0
best_error = np.inf
# Loop through different values of p and k
p = -.1
while p < 2:
	p += .1
	for k in range(13, 20):
		# Interpolate using the current p and k
		dtm_z_1 = idw_interpolate_points_test(x1[ground1], y1[ground1], z1[ground1], power=p, max_neighbors=k)
		#dtm_z_2 = idw_interpolate_points_test(x2[ground2], y2[ground2], z2[ground2], power=p, max_neighbors=k)
		# Calculate error (e.g., RMSE)
		error = np.sqrt(np.nanmean((dtm_z_1 - z1[ground1]) ** 2)) #+ np.nanmean((dtm_z_2 - z2[ground1]) ** 2))

		# Update best parameters if current error is lower
		if error < best_error:
			print(f"New best parameters: p={p}, k={k}, RMSE={error}")
			best_error = error
			best_p = p
			best_k = k
		else:
			print(f"p={p}, k={k}, RMSE={error}")

print(f"Best parameters: p={best_p}, k={best_k}, RMSE={best_error}")
exit(0)
